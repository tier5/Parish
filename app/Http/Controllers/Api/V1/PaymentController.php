...
class PaymentController extends Controller {
    ...
    public function downloadFile(Request $request, $payment_id) {
        ...
        if($payment->file_name)
        {
            ...
            $destinationPath = public_path()."/paymentReceipt/".$payment->file_name;
            //return response()->download($destinationPath);
             $headers = array(
              'Content-Type:'.mime_content_type( $destinationPath ),
            );

         return response()->download($destinationPath, $payment->file_name, $headers);
        }
        else
        {
            Log::error($exception->getMessage());
