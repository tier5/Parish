...
class ReportController extends Controller {
    ...
    function getWeekDates($date, $start_date, $end_date,$attendance,$monetary){
        ...
        foreach ($period as $date) {
            $programes          = (date('l', strtotime($date->format("d-m-Y"))) == 'Sunday')?'Sunday School':((date('l', strtotime($date->format("d-m-Y"))) == 'Wednesday')?'Digging Deep':((date('l', strtotime($date->format("d-m-Y"))) == 'Friday')?'Parish Vigil':null));
            $day['date']        = $date->format("d-m-Y");
            $day['day']         = date('l', strtotime($date->format("d-m-Y")));
            $day['programmes']  = $programes;
